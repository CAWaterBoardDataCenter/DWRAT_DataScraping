...
PRMS <- read.csv(here("ProcessedData/PRMS_2023-10.csv")) #Imports first 22 sub-basin flows that were generated by PRMS model
PRMS$Date <- as.Date(PRMS$Date)
colnames(PRMS)[2:23] <- c(1:22)
Raw_Flows <- merge(PRMS, SRP_monthly, by = "Date") #Contains all 28 sub-basin flows
